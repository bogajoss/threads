            ...
            <div className="px-6 pb-6 pt-8 space-y-8">
                <div className="flex flex-col items-center gap-4">
                    ...
                    >
                        <Avatar className="size-20 border-2 border-zinc-100 dark:border-zinc-800 shadow-md">
                            {avatarFile ? (
                                <AvatarImage src={URL.createObjectURL(avatarFile)} className="object-cover" />
                            ) : (
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-zinc-500 ml-1">Add Members</label>
                    ...
                    {selectedUsers.length > 0 && (
                        <div className="flex flex-wrap gap-2 py-1">
                            {selectedUsers.map(user => (
                                <div key={user.id} className="flex items-center gap-1.5 rounded-full bg-violet-500/10 px-2.5 py-1 text-xs font-bold text-violet-600 dark:bg-violet-500/20">
                                    <Avatar className="size-4">
                                        <AvatarImage src={user.avatar} />
                                        <AvatarFallback>{user.name[0]}</AvatarFallback>
                                    </Avatar>
                                    <span>{user.name}</span>
                                    <button onClick={() => toggleUser(user)} className="hover:text-violet-800">
                                        <X size={12} />
                                    </button>
                                </div>
                            ))}
                                ...
                                return (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            isSelected ? "bg-violet-50 dark:bg-violet-900/20" : "hover:bg-zinc-50 dark:hover:bg-zinc-900"
                                        )}
                                    >
                                        ...
                                        <div className={cn(
                                            ...
                                            isSelected ? "border-violet-500 bg-violet-500 text-white" : "border-zinc-200 dark:border-zinc-700"
                                        )}>
