...
interface ChatInputProps {
    onSendMessage: (text: string, attachments: File[], audioBlob?: Blob, duration?: number) => void
            ...
            {attachments.length > 0 && (
                <div className="flex gap-2 mb-2 px-1">
                    {attachments.map((file, idx) => (
                        <div key={idx} className="relative size-16 rounded-lg overflow-hidden border border-zinc-200">
                            <img src={URL.createObjectURL(file)} className="size-full object-cover" />
                            <button onClick={() => setAttachments(prev => prev.filter((_, i) => i !== idx))} className="absolute top-0 right-0 bg-black/50 text-white p-0.5"><X size={12} /></button>
                        </div>
                    ))}
                </div>
            )}
                            ...
                            <div className="flex items-center gap-0.5 ml-1">
                                <Popover open={isEmojiOpen} onOpenChange={setIsEmojiOpen}>
                                    ...
                                    <PopoverContent className="w-full p-0 border-none bg-transparent shadow-none" side="top" align="start">
                                        <EmojiPicker onEmojiSelect={(emoji: any) => setText(prev => prev + emoji.emoji)} />
                                    </PopoverContent>
                                </Popover>
