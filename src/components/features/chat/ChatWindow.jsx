...
import { ArrowLeft, MoreVertical, Send, Smile, Paperclip, Loader2, X, Check, CheckCheck } from 'lucide-react';
            ...
            <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-white dark:bg-black shrink-0">
                {attachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                        {attachments.map(att => (
                            <div key={att.url} className="relative size-16 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
                ...
                <form onSubmit={handleSend} className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900 p-2 rounded-2xl border border-transparent focus-within:border-violet-500 transition-all">
                    <Popover open={isEmojiOpen} onOpenChange={setIsEmojiOpen}>
                        <PopoverTrigger asChild>
                            <button type="button" className="p-2 text-zinc-500 hover:text-violet-600 transition-colors">
                                <Smile size={20} />
                            </button>
                        </PopoverTrigger>
                        <PopoverContent className="w-fit p-0 border-none bg-transparent shadow-none" side="top" align="start" sideOffset={10}>
                    ...
                    <Button type="submit" className="size-10 !p-0 rounded-xl" disabled={(!text.trim() && attachments.length === 0) || isUploading}><Send size={18} /></Button>
                </form>
            </div>
        </div>
    );
