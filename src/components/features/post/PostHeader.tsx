...
const PostHeader: React.FC<PostHeaderProps> = ({
    ...
    return (
        <div className={`flex items-center justify-between ${isDetail ? "pb-4" : "min-h-[40px] mb-1"}`}>
            <div className="flex items-center gap-x-3 flex-1 min-w-0">
                {showAvatar && (
                    <button className="shrink-0" onClick={handleUserClick}>
                        <Avatar className={`${isDetail ? "size-12" : isComment ? "size-8" : "size-10"} border-0 shadow-sm`}>
                            <AvatarImage
                                src={user.avatar}
                                alt={user.handle}
                        ...
                        <div className="flex min-w-0 max-w-full items-center gap-1.5">
                            <button
                                className={`flex shrink-0 items-center gap-1 font-bold text-zinc-900 hover:underline dark:text-white ${
                                    isDetail ? "text-base sm:text-lg" : isComment ? "text-[14px]" : "text-[15px]"
                                }`}
                            ...
                            >
                                ...
                                {user.verified && (
                                    <VerifiedIcon size={isDetail ? 16 : isComment ? 12 : 14} className="text-blue-500" />
                                )}
                            </button>
