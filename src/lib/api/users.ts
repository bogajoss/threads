 ...
 */
export const fetchProfiles = async (lastTimestamp: string | null = null, limit: number = 10): Promise<User[]> => {
    let query = supabase
        .from("users")
        .select("*")
 ...
 */
export const fetchUserProfile = async (identifier: string, type: "id" | "username" = "id"): Promise<User | null> => {
 ...
 */
export const updateProfile = async (userId: string, fields: ProfileUpdateFields): Promise<void> => {
    ...
    if (fields.avatar || fields.cover) {
        ...
        if (oldProfile) {
            ...
            if (fields.avatar && oldProfile.avatar_url && oldProfile.avatar_url !== fields.avatar) {
                if (oldProfile.avatar_url.includes(supabase.storage.from('media').getPublicUrl('').data.publicUrl)) {
                    await deleteFileFromUrl(oldProfile.avatar_url);
                }
            }
            // Delete old cover if it's a Supabase URL and different from new one
            if (fields.cover && oldProfile.cover_url && oldProfile.cover_url !== fields.cover) {
                if (oldProfile.cover_url.includes(supabase.storage.from('media').getPublicUrl('').data.publicUrl)) {
 ...
 */
export const fetchFollowStats = async (userId: string): Promise<{ followers: number; following: number }> => {
    const { data, error } = await (supabase
        .from("users") as any)
