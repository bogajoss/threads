...
import { fetchProfiles, toggleFollow, searchUsers, updateProfile } from "@/lib/api";
...
export default function OnboardingPage() {
  ...
  if (loading || !currentUser) {
      return <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black"><Loader2 className="w-8 h-8 animate-spin text-zinc-500" /></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-white text-black dark:bg-black dark:text-white flex flex-col items-center justify-center">
      <AnimatePresence mode="wait">
        {step === 1 && (
          <FollowStep key="step1" onNext={handleNext} onBack={handleBack} currentUser={currentUser} />
        )}
        {step === 2 && (
          <HowItWorksStep key="step2" onNext={handleNext} onBack={handleBack} isLoading={false} />
        )}
      </AnimatePresence>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6">
        ...
        <button onClick={onNext} className="flex items-center text-zinc-900 dark:text-white ml-auto hover:opacity-70 transition">
      ...
      <div className="mb-6 md:mb-8 text-center space-y-1 md:space-y-2">
        ...
        <p className="text-zinc-500 text-xs md:text-sm">Build your feed with people and communities you love</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto -mx-2 md:-mx-4 px-2 md:px-4 pb-24 space-y-3 md:space-y-4 scrollbar-hide">
        ...
        {!isLoading && filteredUsers.length === 0 && (
          <div className="text-center text-zinc-500 py-10 text-sm md:text-base">No creators found</div>
        )}
      </div>

      {/* Bottom Action */}
      <div className="fixed bottom-4 md:bottom-8 left-0 right-0 px-4 md:px-8 max-w-2xl mx-auto">
        <Button onClick={followAll} className="w-full text-sm md:text-[15px] h-11 md:h-12" disabled={isFollowingAll || isLoading || filteredUsers.length === 0}>
          {isFollowingAll ? <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin" /> : "Follow all"}
        </Button>
      </div>
    </motion.div>
  );
  ...
  return (
    ...
    >
       ...
       <div className="flex items-center mb-4 md:mb-6">
        <button onClick={onBack} className="flex items-center text-zinc-900 dark:text-white hover:opacity-70 transition">
      ...
      <div className="space-y-6 md:space-y-8 flex-1 overflow-y-auto pb-24 scrollbar-hide">
        ...
        <div className="flex gap-3 md:gap-4">
            ...
            <div className="space-y-1 md:space-y-2 min-w-0">
                <h3 className="font-semibold text-sm md:text-base">Express yourself</h3>
                <p className="text-zinc-500 text-xs md:text-sm leading-relaxed">
                    Share your thoughts, life moments, and connect with communities. Post anything from text to multimedia content.
                </p>
            </div>
        </div>
        ...
        <div className="flex gap-3 md:gap-4">
            ...
            <div className="space-y-1 md:space-y-2 min-w-0">
                <h3 className="font-semibold text-sm md:text-base">Discover & Connect</h3>
                <p className="text-zinc-500 text-xs md:text-sm leading-relaxed">
                    Follow creators, join communities, and engage with content that matters. Build meaningful connections with people worldwide.
                </p>
            </div>
        </div>
        ...
        <div className="flex gap-3 md:gap-4">
            ...
            <div className="space-y-1 md:space-y-2 min-w-0">
                <h3 className="font-semibold text-sm md:text-base">Privacy & Safety</h3>
                <p className="text-zinc-500 text-xs md:text-sm leading-relaxed">
                    Your data is yours. We prioritize privacy and security. Control what you share and who sees it.
                </p>
            </div>
        </div>
      ...
      <div className="fixed bottom-4 md:bottom-8 left-0 right-0 px-4 md:px-8 max-w-2xl mx-auto">
        <Button 
            ...
            disabled={isJoining}
        >
            {isJoining ? <Loader2 className="w-4 h-4 md:w-5 md:h-5 animate-spin" /> : "Join Sysm"}
        </Button>
      </div>
