            ...
            <div className="min-h-screen bg-black text-white selection:bg-blue-500/30 pb-20">
                ...
                <div className="sticky top-0 z-30 bg-black/80 backdrop-blur-md border-b border-white/10">
                    <div className="max-w-6xl mx-auto px-4 h-16 flex items-center gap-4">
                        <Link to="/shop" className="p-2 -ml-2 hover:bg-white/10 rounded-full transition-colors text-neutral-400 hover:text-white">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="bg-[#111] border border-white/5 rounded-2xl p-5 relative overflow-hidden group">
                                    ...
                                    <div className="text-neutral-500 text-xs uppercase tracking-wider font-semibold mb-1">Highest Bid</div>
                                    <div className="text-2xl font-bold flex items-center gap-1">
                                        <span className="text-blue-400">💎</span> {product.price.toLocaleString()}
                                    </div>
                                ...
                                <div className="bg-[#111] border border-white/5 rounded-2xl p-5">
                                    <div className="text-neutral-500 text-xs uppercase tracking-wider font-semibold mb-1">Bid Step</div>
                                ...
                                <div className="bg-[#111] border border-white/5 rounded-2xl p-5">
                                    <div className="text-neutral-500 text-xs uppercase tracking-wider font-semibold mb-1">Minimum Bid</div>
                                    <div className="text-2xl font-bold">
                                        <span className="text-blue-400">💎</span> {(product.minBid || product.price * 1.05).toLocaleString()}
                                    </div>
                                    <div className="text-sm text-neutral-500 mt-1">
                                        ~ ${((product.priceUsd / product.price) * (product.minBid || product.price * 1.05)).toFixed(0).toLocaleString()}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#111] border border-white/5 rounded-2xl overflow-hidden">
                                ...
                                <div className="divide-y divide-white/5">
                                    <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        ...
                                        <a href={`https://t.me/${product.username}`} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline font-medium">
                                            @{product.username}
                                        </a>
                                    </div>
                                    <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-blue-400 hover:underline font-medium cursor-pointer">t.me/{product.username}</span>
                                            <Copy className="w-4 h-4 text-neutral-600 cursor-pointer hover:text-white" />
                                        </div>
                                    </div>
                                    <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <span className="text-neutral-400">TON Web 3.0 Address</span>
                                        <div className="flex items-center gap-2">
                                            <span className="text-blue-400 hover:underline font-medium cursor-pointer">{product.username}.t.me</span>
                                            <Copy className="w-4 h-4 text-neutral-600 cursor-pointer hover:text-white" />
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                {product.bidHistory.map((bid, i) => (
                                                    <tr key={i} className="hover:bg-white/[0.02]">
                                                        <td className="px-6 py-4 font-bold">
                                                            <span className="text-blue-400">💎</span> {bid.price.toLocaleString()}
                                                        </td>
                                                        <td className="px-6 py-4 text-neutral-400">{bid.date}</td>
                                                    ...
                                                    <tr key={i} className="hover:bg-white/[0.02]">
                                                        <td className="px-6 py-4 font-bold">
                                                            {record.price === "Transferred" ? (
                                                                <span className="text-neutral-500 font-medium">{record.price}</span>
                                                            ) : (
                                                                <span><span className="text-blue-400">💎</span> {record.price}</span>
                                                            )}
                                                        </td>
                                                        <td className="px-6 py-4 text-neutral-400">{record.date}</td>
                                                        <td className="px-6 py-4 text-right">
                                                            <span className="font-mono text-blue-400 hover:underline cursor-pointer truncate max-w-[150px] inline-block align-bottom">
                                                                {record.buyer}
                                                            </span>
                            ...
                            <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-900/20 to-purple-900/20 border border-blue-500/20">
                                ...
                                <p className="text-neutral-300 leading-relaxed max-w-2xl">
                                    You can buy this collectible username and use it for your personal profile, group or channel.
                                    Usernames are stored on the TON blockchain as NFTs.
                                </p>
                                <div className="mt-4 flex gap-4">
                                    <Button variant="outline" size="sm" className="bg-black/20 border-white/10 hover:bg-white/10">Learn More</Button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-[#111] border border-white/5 rounded-2xl p-6 sticky top-24">
                                <div className="flex justify-center mb-6">
                                    <div className="p-4 bg-white rounded-xl">
                                        <QRCodeSVG value={`https://t.me/${product.username}`} size={160} />
                                    </div>
                                </div>

                                <h3 className="text-center text-xl font-bold mb-1">Scan to bid</h3>
                                ...
                                <div className="bg-neutral-900 rounded-xl p-4 mb-4">
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className="font-mono font-medium">{product.timeLeft}</span>
                                    </div>
                                ...
                                <Button variant="outline" className="w-full border-white/10 hover:bg-white/5 text-neutral-300">
                                    Make an offer
                                </Button>
                            </div>
                            ...
                            <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
                                ...
                                <Link to="#" className="text-blue-400 text-sm hover:underline flex items-center gap-1">
