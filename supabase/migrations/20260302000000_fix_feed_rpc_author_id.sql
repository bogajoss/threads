-- Fix Feed RPCs to include author ID in author_data
-- This is required for checking follow status and correct navigation
...
BEGIN
  ...
  WITH feed_items AS (
    ...
    SELECT
      ...
      ) ELSE NULL END as community_data,
      FALSE as is_repost,
  ...
  SELECT
    ...
    jsonb_build_object('id', s.u_id, 'username', s.u_name, 'avatar_url', s.u_avatar, 'roles', s.u_roles, 'is_pro', s.u_pro, 'is_verified', s.u_verified) AS author_data,
    s.community_data,
...
BEGIN
  ...
  WITH raw_reels AS (
    SELECT
      ...
      jsonb_build_object('id', u.id, 'username', u.username, 'avatar_url', u.avatar_url, 'roles', u.roles, 'is_pro', u.is_pro, 'is_verified', u.is_verified) as author_data,
      p.content, p.media, p.likes_count, p.comments_count, p.views_count, p.user_id as author_id
    FROM public.posts p
    JOIN public.users u ON p.user_id = u.id
  ...
  SELECT s.id, s.created_at, s.rank_score AS score, s.author_data, s.content, s.media, s.likes_count, s.comments_count, s.views_count, s.author_id as user_id
  FROM scored_reels s
