...
-- Update get_profile_feed to support pinned posts
...
BEGIN
  ...
  WITH feed_items AS (
    ...
    SELECT
      ...
      FALSE as is_pinned, -- Reposts cannot be pinned
  ...
  FROM feed_items f
  WHERE
    (last_item_time IS NULL) -- Page 1: return everything (pinned first due to ORDER BY)
    OR
    (last_item_time IS NOT NULL AND f.is_pinned = FALSE AND f.sort_time < last_item_time) -- Page 2+: return only unpinned posts
  ORDER BY f.is_pinned DESC, f.pinned_at DESC NULLS LAST, f.sort_time DESC
  LIMIT limit_val;
END;
