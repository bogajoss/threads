-- Fix get_profile_feed pagination and sorting for pinned posts
-- Pinned posts should ALWAYS be on top of Page 1.
-- Subsequent pages should only return unpinned posts older than the cursor.
...
BEGIN
  ...
  WHERE
    -- Page 1: last_item_time is NULL, return everything (pinned will be first)
    (last_item_time IS NULL)
    OR
    -- Page 2+: return only UNPINNED items that are older than the cursor
    (last_item_time IS NOT NULL AND f.is_pinned = FALSE AND f.sort_time < last_item_time)
  ORDER BY 
    f.is_pinned DESC, 
...
-- Grant permissions for all possible signatures used in the app
