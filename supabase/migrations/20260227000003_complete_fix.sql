-- Complete fix for get_profile_feed - drop all versions first
-- This handles the "cannot change name of input parameter" error

-- Drop ALL existing versions of get_profile_feed
...
DROP FUNCTION IF EXISTS public.get_profile_feed(UUID, INTEGER, TEXT);
DROP FUNCTION IF EXISTS public.get_profile_feed(UUID, INTEGER, TIMESTAMPTZ, TEXT);

-- Verify all versions are dropped
-- SELECT routine_name, routine_signature FROM information_schema.routines WHERE routine_name = 'get_profile_feed';

-- Create the new version with clear parameter names
...
BEGIN
  ...
  SELECT
    ...
    f.pinned_at
  FROM (
    -- USER'S POSTS (non-reels when filter_mode is NULL or 'posts')
    SELECT
      p.id as f_id,
      p.id as p_id,
    ...
    WHERE
      ...
      AND (
        -- NULL or 'posts' = all posts except reels
        (filter_mode IS NULL OR filter_mode = 'posts') AND COALESCE(p.type, 'text') != 'reel'
        -- 'reel' = only reels
        OR filter_mode = 'reel' AND p.type = 'reel'
        -- 'all' = everything
        OR filter_mode = 'all'
      )
...
GRANT EXECUTE ON FUNCTION public.get_profile_feed(UUID, INTEGER, TEXT) TO authenticated;

-- Verify the function was created
-- SELECT * FROM get_profile_feed('YOUR-USER-ID-HERE', 20, NULL, NULL);
