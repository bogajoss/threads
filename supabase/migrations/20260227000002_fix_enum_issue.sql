-- Fix get_profile_feed - use proper parameter handling
-- 'posts' is NOT a valid post_type enum value, so we handle it as a filter mode
...
CREATE OR REPLACE FUNCTION public.get_profile_feed(
  ...
  filter_mode TEXT DEFAULT NULL  -- NULL or 'posts' = non-reels, 'reel' = only reels, 'all' = everything
)
RETURNS TABLE (
  feed_id UUID,
  id UUID,
...
BEGIN
  ...
  FROM (
    ...
    WHERE
      ...
      AND (
        -- Default/NULL or 'posts' = all posts except reels
